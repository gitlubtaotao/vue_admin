<template>
  <div>
      ...
      <el-row style="margin-top: 10px">
        ...
        <el-dropdown size="mini" type="info" @command="handleCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item command="copyFeeOtherOrder">复制到其他订单</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
    ...
    <el-dialog title="复制费用到其他订单" :visible.sync="centerDialogVisible" width="60%">
      <keep-alive>
        <order-table :search-params="searchOrderMaster" @listenOrderIds="copyFeeOtherOrder" />
      </keep-alive>
      <div slot="footer" class="dialog-footer">
        <el-button @click="centerDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handlerCopyFeeOther">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    searchOrderMaster: function() {
      const scope = {
        instruction_id: parseInt(this.$store.state.orderMaster.receiveClosingUnitId),
        status: 'processing',
        transport_type: this.transport_type
      }
      if (this.payOrReceive === 'receive') {
        scope['receivable_status'] = 'unfinished'
      } else {
        scope['payable_status'] = 'unfinished'
      }
      return scope
    }
  },
  ...
  methods: {
    ...
    deleteFee(row, $index) {
      ...
      this.beforeCheck(rows, function(rows) {
        ...
        for (let i = 0; i < rows.length; i++) {
          if (rows[i].status !== 'init' && rows[i].status !== 'pending' && rows[i].status !== 'verify') {
    ...
    handleCommand(command) {
      if (command === 'copyFee') {
        this.copyFee()
      } else if (command === 'copyFeeOtherOrder') {
        this.copyFeeOtherOrder()
      }
    },
    // 复制费用到其他订单中
    copyFeeOtherOrder(val) {
    ...
    // 处理复制费用到其他订单
    handlerCopyFeeOther() {
      if (this.chooseOrderMasterIds.length === 0) {
        this.$message.error('没有选择记录')
        return
      }
      const feeIds = this.multipleSelection.map((item) => {
        return item.id
      })
      createData('/finance/fees/CopyFee', { fee_ids: feeIds, order_master_ids: this.chooseOrderMasterIds }).then((response) => {
        this.centerDialogVisible = false
        this.$notify({ title: 'Success', message: '复制费用成功', type: 'success', duration: 5000 })
      }).catch(reason => {
      })
    },
    handleCopyFee(data) {
