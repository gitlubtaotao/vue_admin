...
export default {
  ...
  data() {
    return {
      ...
      listLoading: false,
      loading: false,
      columnUrl: '/crm/companies/column?type=customer',
      listQuery: {
        page: 1,
        limit: 20,
        name_nick: undefined,
      ...
      rules: {
        ...
        user_salesman_id: [{ required: true, message: '请输入业务员', trigger: 'blur' }],
      ...
      userCompanies: [],
      userCompanyOptions: [],
      companyTypeOptions: [{ label: '客户', value: 1 }, { label: '客户&供应商', value: 3 }],
      sourceOptions: []
    }
  },
  created() {
  ...
  methods: {
    ...
    filterTable() {
      ...
      getData('/crm/companies/data', this.listQuery).then(response => {
    ...
    getCompany() {
      if (this.usersOptions.length === 0) {
        ...
        this.userCompanyOptions = this.user_companies
      }
    },
    handleBusiness() {
    ...
    handleRoles() {
      return [
        { user_id: this.temp.user_salesman_id, name: 'salesman' },
